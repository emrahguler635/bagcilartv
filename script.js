...
// Haber verilerini saklamak için global değişken (başlangıçta fallback - RSS yüklenince güncellenir)
...
// Finansal veriler (başlangıç değerleri - API'den güncellenecek)
let financeData = {
...
// Finans API - güncel döviz, altın, BIST verilerini çek
const FINANS_API_URL = 'https://finans.truncgil.com/v4/today.json';
const FINANS_PROXIES = [
    `https://corsproxy.io/?${encodeURIComponent(FINANS_API_URL)}`,
    `https://api.allorigins.win/raw?url=${encodeURIComponent(FINANS_API_URL)}`,
    `https://thingproxy.freeboard.io/fetch/${FINANS_API_URL}`,
    FINANS_API_URL // Son çare direkt dene
];

async function fetchFinanceFromAPI() {
    // 1. Ana kaynak: finans.truncgil (proxy üzerinden - CORS sorununu aşar)
    for (const url of FINANS_PROXIES) {
        try {
            ...
            if (typeof data === 'string') {
                try { data = JSON.parse(data); } catch (_) { throw new Error('JSON parse hatası'); }
            }
            if (!data || typeof data !== 'object') throw new Error('Geçersiz veri');
            if (!data.USD && !data.EUR) throw new Error('Beklenen veri yapısı yok');
            // Verileri güncelle
            if (data.USD?.Selling) financeData.dolar = { value: parseFloat(data.USD.Selling), change: (data.USD.Change || 0) >= 0 ? 'up' : 'down' };
            if (data.EUR?.Selling) financeData.euro = { value: parseFloat(data.EUR.Selling), change: (data.EUR.Change || 0) >= 0 ? 'up' : 'down' };
            if (data['14AYARALTIN']?.Selling) financeData.altin = { value: parseFloat(data['14AYARALTIN'].Selling), change: (data['14AYARALTIN'].Change || 0) >= 0 ? 'up' : 'down' };
            else if (data.GRA?.Selling) financeData.altin = { value: parseFloat(data.GRA.Selling), change: (data.GRA.Change || 0) >= 0 ? 'up' : 'down' };
            if (data.GRA?.Selling) financeData.gramAltin = { value: parseFloat(data.GRA.Selling), change: (data.GRA.Change || 0) >= 0 ? 'up' : 'down' };
            if (data.GUMUS?.Selling) financeData.gumus = { value: parseFloat(data.GUMUS.Selling), change: (data.GUMUS.Change || 0) >= 0 ? 'up' : 'down' };
            if (data.ETH?.TRY_Price) financeData.eth = { value: parseFloat(data.ETH.TRY_Price), change: (data.ETH.Change || 0) >= 0 ? 'up' : 'down' };
            else if (data.ETH?.Selling) financeData.eth = { value: parseFloat(data.ETH.Selling), change: (data.ETH.Change || 0) >= 0 ? 'up' : 'down' };
            if (data.BTC?.TRY_Price) financeData.btc = { value: parseFloat(data.BTC.TRY_Price), change: (data.BTC.Change || 0) >= 0 ? 'up' : 'down' };
            else if (data.BTC?.Selling) financeData.btc = { value: parseFloat(data.BTC.Selling), change: (data.BTC.Change || 0) >= 0 ? 'up' : 'down' };
            if (data.XU100?.Selling) financeData.bist = { value: parseFloat(data.XU100.Selling), change: (data.XU100.Change || 0) >= 0 ? 'up' : 'down' };
            console.log('✅ Finans verileri güncellendi:', data.Update_Date || 'güncel');
            return true;
        } catch (err) {
            console.warn('Finans denemesi başarısız:', err.message || err);
            await new Promise(r => setTimeout(r, 300));
        }
    }
    // 2. Yedek: Frankfurter (USD/EUR→TRY) + CoinGecko (BTC/ETH) - CORS destekli
    try {
        const [usdRes, eurRes, cgRes] = await Promise.all([
        ...
        let updated = false;
        if (usdRes.ok) { const d = await usdRes.json(); if (d.rates?.TRY) { financeData.dolar = { value: d.rates.TRY, change: 'up' }; updated = true; } }
        if (eurRes.ok) { const d = await eurRes.json(); if (d.rates?.TRY) { financeData.euro = { value: d.rates.TRY, change: 'up' }; updated = true; } }
        if (cgRes.ok) {
            const cg = await cgRes.json();
            const usdTry = financeData.dolar?.value || 43;
            if (cg.bitcoin?.usd) { financeData.btc = { value: cg.bitcoin.usd * usdTry, change: 'up' }; updated = true; }
            if (cg.ethereum?.usd) { financeData.eth = { value: cg.ethereum.usd * usdTry, change: 'up' }; updated = true; }
        }
        if (updated) { console.log('✅ Finans verileri (yedek API) güncellendi'); return true; }
    } catch (e) { console.warn('Yedek finans API hatası:', e.message); }
    return false;
}
...
// Finansal verileri güncelle (API'den canlı veri çeker)
let financeUpdateIntervalId = null;
async function updateFinanceTicker() {
    ...
    // Önce mevcut/fallback verilerle hemen göster
    createFinanceTicker();
    
    // API'den güncel verileri çek
    const updated = await fetchFinanceFromAPI();
    if (updated) createFinanceTicker();
    
    // Her 3 dakikada bir güncelle (önceden 5 dk idi)
    if (financeUpdateIntervalId) clearInterval(financeUpdateIntervalId);
    financeUpdateIntervalId = setInterval(async () => {
        const ok = await fetchFinanceFromAPI();
        if (ok) createFinanceTicker();
    }, 180000);
}
...
function createNewsTicker() {
    ...
    if (!htmlContent || htmlContent.trim().length < 50) {
        ...
        fallbackShuffled.forEach((news, index) => {
            if (news && news.trim().length > 0) {
                htmlContent += `<span class="news-item">${news}</span>`;
                if (index < fallbackShuffled.length - 1) {
                    htmlContent += `<span class="news-spacer"></span>`;
                }
            }
        });
...
async function updateNewsTicker() {
    // Önce newsScroll elementini kontrol et
    if (!newsScroll) {
        newsScroll = document.getElementById('newsScroll');
    }
    ...
    if (!newsScroll) {
        ...
        // Element bulunamazsa fallback haberleri göster
        setTimeout(() => {
            newsScroll = document.getElementById('newsScroll');
            if (newsScroll) {
                if (!currentNewsData || currentNewsData.length === 0) {
    ...
    // Önce fallback haberleri GARANTİLİ göster (anında görünürlük için)
    ...
    // HEMEN göster - kesinlikle haberler görünsün
    createNewsTicker();
    
    // Eğer haberler görünmüyorsa tekrar dene
    setTimeout(() => {
        if (!newsScroll || !newsScroll.innerHTML || newsScroll.innerHTML.trim().length < 50) {
            console.warn('Haberler görünmüyor, tekrar yükleniyor...');
            if (!currentNewsData || currentNewsData.length === 0) {
                currentNewsData = [...fallbackNews];
            }
            createNewsTicker();
    ...
    try {
        ...
        if (data.contents) {
            ...
            const newsItems = [];
                ...
                // Son 72 saat içindeki haberleri filtrele (daha fazla güncel haber için)
                const now = new Date();
            ...
            items.forEach((item, index) => {
                    ...
                    const title = item.querySelector('title')?.textContent || '';
                        ...
                        // Kaynak etiketlerini temizle
                            ...
                            cleanTitle = cleanTitle.replace(/^(Haber|News|Gündem|Son Dakika|BREAKING|Son dakika)/i, '').trim();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initFinanceTicker() {
        ...
        if (financeTicker) {
            console.log('💰 Finansal ticker elementi bulundu, yükleniyor...');
            updateFinanceTicker();
            ...
            if (refreshBtn) {
                refreshBtn.addEventListener('click', async () => {
                    ...
                    await updateFinanceTicker();
                    refreshBtn.classList.remove('loading');
                    refreshBtn.disabled = false;
                });
            }
            console.log('✅ Finansal ticker başlatıldı (API güncellemesi arka planda)');
            return true;
        }
        return false;
    }
